        ...
        if reports_dir.exists():
            for rpt_file in sorted(reports_dir.glob("*_report.json")):
                try:
                    ...
                    heatmap_file = model_dir / "output" / "annotated" / f"{stem}_heatmap.png"
                    parts.append({
                        "report": rep,
                        ...
                        # None (not a guessed URL) when the sample was
                        # refused before scoring (e.g. registration
                        # confidence too low) -- there is no valid
                        # heatmap to show, and the frontend falls back
                        # to the annotated layer in that case.
                        "heatmap_image_url": (
                            f"/data/models/{model_name}/output/annotated/{stem}_heatmap.png"
                            if heatmap_file.exists() else None
                        ),
                    })
                except Exception:
                    pass
...
"""
            ...
            # Heatmaps are generated by run_inspection.py itself (above) --
            # from the same per-pixel detector-agreement signal the defect
            # list is extracted from, so they can never disagree with the
            # annotated boxes, and they're already registered/part-masked
            # via the same pipeline that handles sample size mismatches
            # (register()'s subcrop template-matching), so no separate
            # resize step is needed here either. A previous version of
            # this dashboard recomputed a second, disconnected heatmap
            # here from a bare unregistered/unmasked cv2.absdiff rescaled
            # with NORM_MINMAX, which made background and sensor noise
            # look identical to a real defect and made color-only defects
            # invisible despite being flagged CRITICAL in the defect list.
            # Removed in favor of the engine-generated one.
